...
if __name__=='__main__':
    ...
    while True:
        ...
        if random_first > 0.5:
            ...
            board, _, done = tic_tac_toe.step('o', possible_play)
            input_array, board_list = board2list(board)
        while not done:
            action = policy.select_action(input_array, memory_game)
            action = action2place(action)
            board, reward, done = tic_tac_toe.step('x', action)
            input_array, board_list = board2list(board)
            if not done:
                possible_play = random_play(board_list)
                board, reward, done = tic_tac_toe.step('o', possible_play)
                input_array, board_list = board2list(board)
                reward = -reward
        ...
        if samples > BATCH_SIZE:
            ...
            samples = 0
            print('Win Rate: {:.2%}'.format(winner_dict['x']/episode))
        episode += 1
