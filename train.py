...
<<<<<<< HEAD
        ...
        ppo_opponent = False
        random_start = np.random.random()
        if random_start:
            choice = np.array([np.random.choice(np.arange(9))])
            place = action2place(choice)
            board, reward, done, reason = tic_tac_toe.step(second_player, place)
        ...
        while not done:
            ...
            if not done:
                input_array, board_list = board2list(tic_tac_toe.board, main=second_player, opponent=first_player)
