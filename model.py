...
class ActorCritic(nn.Module):
    ...
    def act(self, state, memory, evaluation=False):
        ...
        if evaluation:
            prohibited_states = torch.logical_or(state[:, :9], state[:, 9:])
            prohibited_states = torch.logical_not(prohibited_states)
            action = torch.argmax(action_probabilities*prohibited_states, dim=1)
        else:
            action = dist.sample()
